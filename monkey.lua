--[[
  mobs_mini_animals: Monkey Entity Definition
  
  AI Behavior Rationale:
  Playful primate leaping between trees and ground in tropical biomes.
  
  Grass-Eating Justification:
  NON-GRAZER. Monkeys eat fruits/nuts/insects, not ground pasture grass.
  Zero grass eating behavior enabled. Breeding is not gated by grass consumption.
--]]

local S = core.get_translator("mobs_mini_animals")

mobs:register_mob("mobs_mini_animals:monkey", {
	type = "animal",
	pathfinding = 1,
	passive = false,
	damage = 3,
	hp_min = 6,
	hp_max = 12,
	fear_threshold = 0.5,
	curiosity = 0.5,

	attack_type = "dogfight",
	reach = 1,
	pathfinding = 1,

	armor = 100,
	collisionbox = {-0.4, 0, -0.4, 0.4, 0.8, 0.4},
	visual = "mesh",
	visual_size = {x = 5.175, y = 5.175},
	rotate = 180,
	mesh_animation = "idle",
	mesh = "animal-monkey.glb",
	textures = {
		{"colormap.png"}
	},
	makes_footstep_sound = true,
	sounds = {},
	walk_velocity = 1.5,
	run_velocity = 3.0,
	jump = true,
	jump_height = 3.5,
	stepheight = 0.6,
	pushable = true,
	-- GLB Animation Mapping
	animation = {
		speed_normal = 1,
		stand_start = 0.05, stand_end = 0.95, stand_speed = 1,
		walk_start = 1.05, walk_end = 1.45, walk_speed = 1,
		run_start = 1.55, run_end = 1.95, run_speed = 1,
		eat_start = 2.05, eat_end = 2.80, eat_speed = 1,
	},
	drops = {
		{name = mobs_mini_animals.meat_item, chance = 1, min = 1, max = 2},
	},
	replace_rate = 10,
	replace_what = {
		{"group:grass", "air", 0},
		{"mcl_core:dirt_with_grass", "mcl_core:dirt", -1},
		{"default:dirt_with_grass", "default:dirt", -1},
	},
	on_replace = function(self, pos, oldnode, newnode)
		self:set_animation("eat", true)
		if not self.child and (self.hornytimer or 0) == 0 and math.random(1, 10) == 1 then
			self.horny = true
		end
	end,
	on_breed = function(self, ent)
		return mobs_mini_animals.on_breed_custom(self, ent)
	end,
	on_rightclick = function(self, clicker)
		if mobs:capture_mob(self, clicker, 0, 80, 0, true, nil) then
			return
		end
	end,
})

-- Spawning configuration: Spawns in jungle biomes
if not mobs.custom_spawn_animal then
	mobs:spawn({
		name = "mobs_mini_animals:monkey",
		nodes = {"mcl_core:dirt_with_grass", "default:dirt_with_grass", "group:leaves"},
		min_light = 10,
		interval = 30,
		chance = 2000,
		min_height = 1,
		max_height = 100,
		day_toggle = true,
	})
end

mobs:register_egg("mobs_mini_animals:monkey", S("Monkey"), "inv_animal-monkey.png")
